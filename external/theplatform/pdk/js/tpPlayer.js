...
d.href="#";
var g=this;
var h=function(j){var i=g.videoClicked(j);
...
for(;
c<g;
c++){var j=this.playlist.chapters.chapters[c];
var e=this.playlist.chapters.chapters[c+1];
if(j&&f>=j.startTime&&f<=j.endTime){tpDebug("selecting chapter "+c);
a=j;
break
}else{if(e&&f>=j.endTime&&f<=e.startTime){tpDebug("selecting chapter "+(c+1));
a=e;
f=a.startTime;
break
...
return b
}else{var k=Math.round(this.video.currentTime*1000);
var h=Math.round(this.video.duration*1000);
return{globalDataType:"com.theplatform.pdk.data::TimeObject",mediaTime:k,currentTime:k,currentTimeAggregate:k,duration:h,durationAggregate:h,percentComplete:(k/h)*100,percentCompleteAggregate:(k/h)*100,isAggregate:false,isLive:false}
...
function a(j){if(!j.releaseID||j.releaseID.length==0){return
}var g=c(j);
for(var h=0;
h<e.clipTrackingInfo.length;
h++){if(e.clipTrackingInfo[h].releaseId==j.releaseID&&e.clipTrackingInfo[h].smilIndex==j.smilIndex){if(j.isAd){g.requestCount=1;
break
}else{return
